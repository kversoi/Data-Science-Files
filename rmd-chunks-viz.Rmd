...
No code lines are evaluated when `eval = FALSE`. The code is passed through the Console without being evaluated and displayed as output.
````
```{r, eval = FALSE}`r ''`
1 + 1
```
...
````
```{r}`r ''`
# This is an outrageously long comment... For no real reason... Except the fact that I need to illustrate a specific point.
...
### I hope you engjoyed comparing the output and code!  Again, this is a reference document, so feel free to come check it out as often as you need to help you build better code!